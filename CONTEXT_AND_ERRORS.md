...
**Проблема:**
- Первые 2 итерации редизайна сохранили порядок `hero → cards → trust → process → cards → form`
- Изменили только цвета, размеры, CSS классы
- Владелец: "это тот же самый сайт"

**Причина:**
- Агенты читали существующий HTML как template
- Инструкции были про "улучшение", а не "переписать с нуля"
- Taste anti-slop правила изучены, но не применены жёстко

**Решение:**
- Новая главная должна иметь радикально другую композицию:
  - `hero-full` (large split-screen) вместо centered hero
  - `direction-bar` (inline choice) вместо cards секции
  - `case-feature` (editorial spread) вместо grid cards
  - `facts-bar` (horizontal strip) вместо trust tiles
  - `timeline` (horizontal flow) вместо process cards
  - `contact-section` (profile + form) вместо standalone form
- Страницы направлений должны быть editorial: numbered list, не cards; timeline, не process blocks
- Portfolio должен быть photo archive с mixed ratios, не SaaS grid

**Проверка:**
Спроси себя: "Если обрезать header/footer, смогу ли я отличить новую страницу от старой по композиции?"
